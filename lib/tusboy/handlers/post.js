...
module.exports =
  (store, { getKey = () => {}, maxUploadLength = Infinity }) =>
  async (req, res, next) => {
    const tus = req.tus;
    ...
    if (!defer && !('uploadLength' in tus)) {
      return next(errors.preconditionError('Missing Upload-Length header'));
    }
    // TODO: make sure store supports defer length
    if (defer && !store.supportsDeferredLength) {
      return next(
        errors.preconditionError('Store does not support creation-defer-length extension')
      );
    }
    if (defer && 'uploadLength' in tus) {
      return next(errors.preconditionError('Choose one of Upload-Length OR Upload-Defer-Length'));
    }

    if ('uploadLength' in tus && uploadLength > maxUploadLength) {
      res.set('Tus-Max-Size', maxUploadLength);
      return next(
        errors.entityTooLarge(
