...
/**
 * Sanitizes returnTo redirect parameters to prevent Open Redirect vulnerabilities.
 */
function sanitizeReturnTo(returnToUrl, defaultUrl = '/') {
  if (!returnToUrl || typeof returnToUrl !== 'string') return defaultUrl;
  const trimmed = returnToUrl.trim();
  if (
    !trimmed.startsWith('/') ||
    trimmed.startsWith('//') ||
    trimmed.startsWith('/\\') ||
...
function authorizeBucketSso(db, config) {
  return function (req, res, next) {
    ...
    if (!user) {
      ...
      const cleanReturnTo = sanitizeReturnTo(req.originalUrl, baseUrl);
      ...
      return res.status(401).json({
        ...
        loginUrl: `${baseUrl}auth/sso/login/${encodeURIComponent(meta.org)}?returnTo=${encodeURIComponent(cleanReturnTo)}`,
      });
    }
    ...
    if (userOrg !== requiredOrg) {
      ...
      return res.status(403).json({
        ...
        userOrg,
        message: 'Access Denied: Your current SSO account is not authorized to access these files.',
      });
    }
