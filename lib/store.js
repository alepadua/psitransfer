...
class Store {
  ...
  async create(fid, opts = {}) {
    ...
    await fsp.mkdir(path.dirname(this.getFilename(fid)), { recursive: true });
    await fsp.writeFile(this.getFilename(fid) + '.json', JSON.stringify(Object.assign(opts, {
      isPartial: true
    })));
    return {uploadId: fid};
  }
  ...
  async update(fid, data) {
    ...
    await fsp.writeFile(this.getFilename(fid) + '.json', JSON.stringify(data));
    return data;
  }
  ...
  async info(fid) {
    try {
      const raw = await fsp.readFile(this.getFilename(fid) + '.json', 'utf8');
  ...
  async append(fid, readStream, offset) {
    ...
    const ws = fs.createWriteStream(this.getFilename(fid), {flags: 'a', start: offset});
    ...
    const ret = new Promise((resolve, reject) => {
      ws.on('finish', async() => {
        try {
          ...
          await fsp.writeFile(this.getFilename(fid) + '.json', JSON.stringify(info));
  ...
  createReadStream(fid, start, end, cb) {
    ...
    return fs.createReadStream(this.getFilename(fid), {start, end});
  }
